...
public class PhysicsEngine : IPhysicsEngine
{
    ...
    private static Vector3 TruncateVelocity(double terminalVelocity, Vector3 velocity)
    {
        ...
        lock (_entityLock)
        {
            foreach (var entity in _entities)
            {
                if (entity.BeginUpdate())
                {
                    ...
                    var move = new Ray(entity.Position, velocity * seconds);

                    double nearestCollision;
                    GlobalVoxelCoordinates? collisionBlock;
                    var collisionFace = BlockFace.NegativeX;
                    var nearestCollisionFace = collisionFace;
                    BoundingBox? collisionTarget = null;
                    var collisionCount = 0;
                    var hadCollision = true;

                    var testBox = GetAABMoveBox(entity.BoundingBox, move.Direction);
                    var xmin = (int) Math.Floor(testBox.Min.X);
                    var xmax = (int) Math.Ceiling(testBox.Max.X - 1);
                    var ymin = (int) Math.Floor(testBox.Min.Y);
                    var ymax = (int) Math.Ceiling(testBox.Max.Y - 1);
                    var zmin = (int) Math.Floor(testBox.Min.Z);
                    var zmax = (int) Math.Ceiling(testBox.Max.Z - 1);
                    ...
                    while (collisionCount < 3 && hadCollision)
                    {
                        ...
                        for (var y = ymin; y <= ymax; y++)
                        {
                            ...
                            var target = GetBoundingBox(_dimension, coords);
                            ...
                            var expandedTarget = target.Value.Expand(entity.Size);

                            var collisionDistance = double.MaxValue;

                            if (move.Intersects(expandedTarget, ref collisionDistance, ref collisionFace) &&
                                collisionDistance < nearestCollision)
                            {
                                hadCollision = true;
                        ...
                        if (collisionBlock is not null)
                        {
                            ...
                            var distance = move.Direction;

                            switch (nearestCollisionFace)
                            {
                                case BlockFace.NegativeX:
                                    distance.X = collisionTarget!.Value.Min.X - move.Position.X -
                                ...
                                case BlockFace.PositiveX:
                                    distance.X = collisionTarget!.Value.Max.X - move.Position.X +
                                                 (entity.Size.Width * 0.5);
                                ...
                                case BlockFace.NegativeZ:
                                    distance.Z = collisionTarget!.Value.Min.Z - move.Position.Z -
                                                 (entity.Size.Depth * 0.5);
                                ...
                                case BlockFace.PositiveZ:
                                    distance.Z = collisionTarget!.Value.Max.Z - move.Position.Z +
                                                 (entity.Size.Depth * 0.5);
    ...
    public bool IsGrounded(IEntity entity)
    {
        ...
        var xmin = (int) Math.Floor(bb.Min.X);
        var xmax = (int) Math.Ceiling(bb.Max.X - 1);
        var zmin = (int) Math.Floor(bb.Min.Z);
        var zmax = (int) Math.Ceiling(bb.Max.Z - 1);
        var y = bb.Min.Y;

        var ySupport = (int) (y == (int) Math.Floor(y)
            ? y - 1
            : Math.Floor(y));
        ...
        for (var z = zmin; z <= zmax; z++)
        {
            ...
            var support = GetBoundingBox(_dimension, coords);
            ...
            var supportTop = support.Value.Max.Y + ySupport;
            var supportBottom = support.Value.Min.Y + ySupport;
    ...
    private static BoundingBox GetAABMoveBox(BoundingBox bb, Vector3 move)
    {
        var curMin = bb.Min;
        var curMax = bb.Max;
    ...
    private static BoundingBox? GetBoundingBox(IDimension dimension, GlobalVoxelCoordinates coordinates)
    {
