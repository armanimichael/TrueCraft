...
public class GlobalVoxelCoordinates : IEquatable<GlobalVoxelCoordinates>
{
    ...
    public static GlobalVoxelCoordinates GetGlobalVoxelCoordinates(
        GlobalChunkCoordinates chunk,
        LocalVoxelCoordinates local
    )
    {
        var x = (chunk.X * WorldConstants.ChunkWidth) + local.X;
        var z = (chunk.Z * WorldConstants.ChunkDepth) + local.Z;
    ...
    /// </summary>
    public double DistanceTo(GlobalVoxelCoordinates other) => Math.Sqrt(
        Square(other.X - X) +
        Square(other.Y - Y) +
        Square(other.Z - Z)
    );
    ...
    {
        var dx = other.X - (X + 0.5);
        var dy = other.Y - (Y + 0.5);
        var dz = other.Z - (Z + 0.5);
    ...
    public static bool operator !=(GlobalVoxelCoordinates? a, GlobalVoxelCoordinates? b) => !(a == b);
    ...
    public GlobalVoxelCoordinates Add(Vector3i other) => new(X + other.X, Y + other.Y, Z + other.Z);

    public static GlobalVoxelCoordinates operator +(GlobalVoxelCoordinates c, Vector3i v) => c.Add(v);

    public static GlobalVoxelCoordinates operator +(Vector3i v, GlobalVoxelCoordinates c) => c.Add(v);

    public static Vector3i operator -(GlobalVoxelCoordinates l, GlobalVoxelCoordinates r) => new(l.X - r.X, l.Y - r.Y, l.Z - r.Z);

    public static GlobalVoxelCoordinates operator -(GlobalVoxelCoordinates arg) => new(-arg.X, -arg.Y, -arg.Z);

    #endregion

    #region Conversion operators

    public static explicit operator GlobalVoxelCoordinates(GlobalColumnCoordinates a) => new(a.X, 0, a.Z);

    public static explicit operator GlobalVoxelCoordinates(Vector3 a) => new(
        (int) a.X,
        (int) a.Y,
        (int) a.Z
    );
    ...
    /// </returns>
    public static explicit operator GlobalVoxelCoordinates(GlobalChunkCoordinates a) => new(WorldConstants.ChunkWidth * a.X, 0, WorldConstants.ChunkDepth * a.Z);
    ...
    /// </summary>
    public static readonly GlobalVoxelCoordinates Zero = new(0, 0, 0);

    /// <summary>
    /// Coordinates with x = y = z = 1.
    /// </summary>
    public static readonly GlobalVoxelCoordinates One = new(1, 1, 1);
    ...
    /// <returns></returns>
    public override bool Equals(object? obj) => Equals(obj as GlobalVoxelCoordinates);
