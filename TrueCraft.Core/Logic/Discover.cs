...
public class Discover : IDiscover
{
    ...
    public Discover()
    {
        ...
        using (var xsd = thisAssembly.GetManifestResourceStream("TrueCraft.Core.Assets.TrueCraft.xsd")!)
        {
            _doc.Schemas.Add(XmlSchema.Read(xsd, null)!);
        }

        using (var sz = thisAssembly.GetManifestResourceStream("TrueCraft.Core.Assets.TrueCraft.xml.gz")!)
    ...
    {
        var blockRepository = BlockRepository.Init(discoverer);
        var itemRepository = ItemRepository.Init(discoverer);
        var craftingRepository = CraftingRepository.Init(discoverer);
    ...
    public virtual void DiscoverItemProviders(IRegisterItemProvider repository)
    {
        ...
        var thisAssembly = GetType().Assembly;
        var typeItemProvider = typeof(ItemProvider);
    ...
    public virtual void DiscoverRecipes(IRegisterRecipe repository)
    {
        ...
        var truecraft = _doc.ChildNodes.OfType<XmlNode>()
                            .Where<XmlNode>(n => n.LocalName == "truecraft")
                            .First<XmlNode>();

        var recipes = truecraft.ChildNodes.OfType<XmlNode>()
                               .Where<XmlNode>(n => n.LocalName == "recipes")
                               .First<XmlNode>();
