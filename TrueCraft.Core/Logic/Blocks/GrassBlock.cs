...
public class GrassBlock : BlockProvider
{
    ...
    public override string GetDisplayName(short metadata) => "Grass";

    public override SoundEffectClass SoundEffect => SoundEffectClass.Grass;

    public override Tuple<int, int> GetTextureMap(byte metadata) => new(0, 0);

    protected override ItemStack[] GetDrop(BlockDescriptor descriptor, ItemStack item) => new[] { new ItemStack(DirtBlock.BlockID, 1) };

    private static void ScheduledUpdate(IDimension dimension, GlobalVoxelCoordinates coords)
    {
    ...
    private static void TrySpread(IMultiplayerServer server, IDimension dimension, GlobalVoxelCoordinates coords)
    {
        if (!dimension.IsValidPosition(coords + Vector3i.Up))
        {
            return;
        ...
        for (int i = 0, j = MathHelper.Random.Next(GrowthCandidates.Length); i < GrowthCandidates.Length; i++, j++)
        {
            ...
            if (id == DirtBlock.BlockID)
            {
                ...
                if (grow)
                {
                    dimension.SetBlockID(candidate, BlockID);
