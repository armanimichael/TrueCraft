...
public abstract class CraftingBenchWindow<T> : Window<T>, ICraftingBenchWindow<T>
    where T : ISlot
{
    ...
    public CraftingBenchWindow(
        ...
        base(
            itemRepository,
            windowID,
            Windows.WindowType.CraftingBench,
            name,
            new ISlots<T>[]
            { GetSlots(itemRepository, craftingRepository, slotFactory, width, height), mainInventory, hotBar }
        )
    {
    ...
    private static CraftingArea<T> GetSlots(IItemRepository itemRepository, ICraftingRepository craftingRepository, ISlotFactory<T> slotFactory, int width, int height)
        => new(itemRepository, craftingRepository, slotFactory, width, height);

    public override bool IsOutputSlot(int slotIndex) => slotIndex == 0;
