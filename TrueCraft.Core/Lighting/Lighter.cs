...
public abstract class Lighter : ILighter
{
    ...
    private void DoSkyLightOperation(
    ...
    /// <param name="mode"></param>
    /// <param name="lightLevel"></param>
    private void DoBlockUpdateLightOperation(
        GlobalVoxelCoordinates seed,
        LightingOperationMode mode,
        byte lightLevel
    )
    ...
    )
    {
        ...
        // respectively.
        var xs = (lightLevel * 2) - 1;
        var ys = xs;
        var zs = xs;

        // xo, yo, zo are the coordinates of the bottom northwest corner of the box with the
        // seed at its centre.
        var xo = seed.X - lightLevel + 1;
        var yo = seed.Y - lightLevel + 1;
        var zo = seed.Z - lightLevel + 1;
        ...
        var finished = new Bool3D(xs, ys, zs, false);
        var localLight = lightLevel;
        var stack = new Stack<GlobalVoxelCoordinates>();
        ...
        while (stack.Count > 0)
        {
            ...
            {
                foreach (var v in Vector3i.Neighbors6)
                {
                    var neighborLight = localLight;
                    var neighbor = coords + v;
                    var block = _dimension.BlockRepository.GetBlockProvider(_dimension.GetBlockID(neighbor));
                    var opacity = block?.LightOpacity ?? 0;
    ...
    /// </remarks>
    protected static bool LightVoxel(
    ...
    protected byte GetSkyLight(GlobalVoxelCoordinates coordinates) => _dimension.GetSkyLight(coordinates);
    ...
    /// <param name="lightLevel"></param>
    protected void SetSkyLight(GlobalVoxelCoordinates coordinates, byte lightLevel) => _dimension.SetSkyLight(coordinates, lightLevel);
    ...
    protected byte GetBlockLight(GlobalVoxelCoordinates coordinates) => _dimension.GetBlockLight(coordinates);
    ...
    /// <param name="lightLevel"></param>
    protected void SetBlockLight(GlobalVoxelCoordinates coordinates, byte lightLevel) => _dimension.SetBlockLight(coordinates, lightLevel);
}