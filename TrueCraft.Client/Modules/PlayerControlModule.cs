...
public class PlayerControlModule : InputModule
{
    ...
    public override bool KeyDown(GameTime gameTime, KeyboardKeyEventArgs e)
    {
        switch (e.Key)
        {
            ...
            case Keys.Space:
                ...
                {
                    _game.Client.Velocity += Core.Vector3.Up * GameConstants.JumpVelocity;
                }
    ...
    public override bool MouseButtonDown(GameTime gameTime, MouseButtonEventArgs e)
    {
        switch (e.Button)
        {
            ...
            case MouseButton.Right:
                ...
                var heldItem = _game.Client.Hotbar[_game.Client.HotbarSelection].Item;

                _game.Client.QueuePacket(
                    new PlayerBlockPlacementPacket(
                        _game.HighlightedBlock.X,
                        (sbyte) _game.HighlightedBlock.Y,
                        _game.HighlightedBlock.Z,
    ...
    private void BeginDigging(GlobalVoxelCoordinates target)
    {
        ...
        _game.EndDigging = _game.StartDigging.AddMilliseconds(
            BlockProvider.GetHarvestTime(
                _serviceLocator,
                block,
                _game.Client.Hotbar[_game.Client.HotbarSelection].Item.ID,
                out damage
            )
        );

        _game.Client.QueuePacket(
            new PlayerDiggingPacket(
                PlayerDiggingPacket.Action.StartDigging,
                _game.TargetBlock.X,
                (sbyte) _game.TargetBlock.Y,
                _game.TargetBlock.Z,
                _game.HighlightedBlockFace
            )
        );
    ...
    {
        var coords = (GlobalVoxelCoordinates) _game.Client.BoundingBox.Min.Floor();
        ...
        var provider = _serviceLocator.BlockRepository.GetBlockProvider(target);
        ...
        var effect = $"footstep.{Enum.GetName(provider.SoundEffect)?.ToLower()}";
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (gamePad.IsConnected && _game.HighlightedBlock is not null && gamePad.Triggers.Left > 0.5f &&
            _gamePadState.Triggers.Left < 0.5f)
        {
            var heldItem = _game.Client.Hotbar[_game.Client.HotbarSelection].Item;
        ...
        if (digging)
        {
            ...
            else // Currently digging a block
            {
                ...
                if (ReferenceEquals(target, null)) // Cancel
                {
                    _game.StartDigging = DateTime.MinValue;
                    _game.EndDigging = DateTime.MaxValue;
                    _game.TargetBlock = null;
        ...
        {
            var newVelocity = XVector3.Transform(
                -delta,
