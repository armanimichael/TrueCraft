...
public class ChunkModule : IGraphicalModule
{
    ...
    private Lighting WorldLighting { get; }
    ...
    {
        WorldLighting.EnqueueOperation(
            new TrueCraft.Core.BoundingBox(
                (Core.Vector3) e.Position,
                (Core.Vector3) (e.Position + Vector3i.One)
            ),
            false
        );
        ...
        var posA = new Core.Vector3(e.Position.X, 0, e.Position.Z);
        var posB = new Core.Vector3(e.Position.X + 1, WorldConstants.Height, e.Position.Z + 1);
    ...
    private void Game_Client_ChunkModified(object? sender, ChunkEventArgs e) => ChunkModeller.Enqueue(e.Chunk, true);

    private void Game_Client_ChunkLoaded(object? sender, ChunkEventArgs e) => ChunkModeller.Enqueue(e.Chunk);

    private void MeshCompleted(object? sender, ModellerEventArgs<IChunk> e) => _incomingChunks.Add((ChunkMesh) e.Result);

    private void UnloadChunk(IChunk chunk) => _game.Invoke(
        () =>
        {
        ...
        switch (e.PropertyName)
        {
            case "Position":
                var sorter = new ChunkSorter(
                    new GlobalVoxelCoordinates(
                        (int) _game.Client.Position.X,
                        0,
                        (int) _game.Client.Position.Z
                    )
                );
    ...
    private sealed class ChunkSorter : Comparer<ChunkMesh>
    {
        ...
        {
            var distX = ((GlobalVoxelCoordinates) x!.Chunk.Coordinates).DistanceTo(_camera);
            var distY = ((GlobalVoxelCoordinates) y!.Chunk.Coordinates).DistanceTo(_camera);
    ...
    public void Update(GameTime gameTime)
    {
        ...
        while (_incomingChunks.TryTake(out mesh))
        {
            ...
            if (mesh is not null)
            {
                ...
                {
                    var existing = _chunkMeshes.FindIndex(m => m.Chunk.Coordinates == mesh.Chunk.Coordinates);
    ...
    private static readonly BlendState ColorWriteDisable = new()
                                                           {
    ...
    public void Draw(GameTime gameTime)
    {
        ...
        for (var i = 0; i < _chunkMeshes.Count; i++)
        {
