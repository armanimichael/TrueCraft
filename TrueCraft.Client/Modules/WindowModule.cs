...
public class WindowModule : InputModule, IGraphicalModule
{
    ...
    private static readonly Rectangle InventoryWindowRect = new(0, 0, 176, 166);
    private static readonly Rectangle CraftingWindowRect = new(0, 0, 176, 166);
    private static readonly Rectangle _furnaceWindowRect = new(0, 0, 176, 166);
    ...
    /// </remarks>
    private static readonly Rectangle _chestWindowRect = new(0, 0, 176, 166);
    ...
    /// Texture.  In turn, this size is chosen for compatibility with MineCraft.
    /// </remarks>
    private static readonly Rectangle _doubleChestWindowRect = new(0, 0, 176, 299);

    public void Draw(GameTime gameTime)
    {
        if (ReferenceEquals(Game.Client.CurrentWindow, null))
        {
            return;
        }
        ...
        switch (Game.Client.CurrentWindow.Type)
        {
            ...
            case WindowType.Chest:
                var doubleChest = ((IChestWindow<ISlot>) Game.Client.CurrentWindow).DoubleChest;

                var texture = doubleChest
                    ? _doubleChest
                    : _chest;

                var chestRect = doubleChest
                    ? _doubleChestWindowRect
                    : _chestWindowRect;

                SpriteBatch.Draw(
                    texture,
                    new Vector2(
                        (Game.GraphicsDevice.Viewport.Width / 2) - Scale(chestRect.Width / 2),
        ...
        if (provider is not null && provider.GetIconTexture((byte) HeldItem.Metadata) == null &&
            provider is IBlockProvider)
        {
        ...
        if (SelectedSlot >= 0)
        {
            ...
            if (!item.Empty)
            {
                var p = _itemRepository.GetItemProvider(item.ID)!; // item is known to not be Empty
                var size = Font.MeasureText(p.GetDisplayName(item.Metadata));
    ...
    private sealed class MyHeldItem : IHeldItem
    {
    ...
    public override bool MouseButtonDown(GameTime gameTime, MouseButtonEventArgs e)
    {
        ...
        var rightClick = e.Button == MouseButton.Right;

        var shiftClick = Keyboard.GetState().IsKeyDown(Keys.LeftShift) ||
                         Keyboard.GetState().IsKeyDown(Keys.RightShift);

        ActionConfirmation? action;

        action = ((IClickHandler) Game.Client.CurrentWindow).HandleClick(
            SelectedSlot,
            rightClick,
            shiftClick,
            new MyHeldItem(this)
        ...
        IPacket packet = new ClickWindowPacket(
            id,
            SelectedSlot,
            rightClick,
            action.ActionNumber,
    ...
    public override bool MouseButtonUp(GameTime gameTime, MouseButtonEventArgs e) => Game.Client.CurrentWindow != null;
    ...
    {
        // TODO fix hard-coded constants
        var window = (InventoryWindow) Game.Client.CurrentWindow!;
    ...
    {
        var window = (ChestWindow) Game.Client.CurrentWindow!;
        var bSingleChest = !window.DoubleChest;

        var rect = bSingleChest
            ? _chestWindowRect
            : _doubleChestWindowRect;
        ...
        //       Hotbar inventory.
        var yPlayerInventory = bSingleChest
            ? 84
            : 140;

        var deleteme = window.ChestInventory;
    ...
    {
        var furnaceProgress = (IFurnaceProgress) Game.Client.CurrentWindow!;
        var vp = Game.GraphicsDevice.Viewport;
        var x = (int) ((vp.Width - Scale(_furnaceWindowRect.Width)) / 2);
        var y = (int) ((vp.Height - Scale(_furnaceWindowRect.Height)) / 2);
        int progress;
        ...
        var smeltingProgress =
            new Rectangle(x + (int) Scale(80), y + (int) Scale(40), (int) Scale(progress), (int) Scale(6));

        var smeltingSource = new Rectangle(177, 20, progress, 6);
        ...
        var flameHeight = new Rectangle(
            x + (int) Scale(58),
            y + (int) Scale(36 + progress),
            (int) Scale(11),
            (int) Scale(14 - progress)
        );

        var flameSource = new Rectangle(178, 0 + progress, 11, 14 - progress);
        SpriteBatch.Draw(_furnace, flameHeight, flameSource, Color.White);
    }

    private void DrawFurnaceWindow(RenderStage stage)
    {
        var window = (FurnaceWindow) Game.Client.CurrentWindow!;
        ...
        DrawWindowArea(
            window.MainInventory,
            window.MainSlotIndex,
            8,
            83,
            _furnaceWindowRect,
            stage
    ...
    )
    {
        ...
        for (var i = 0; i < area.Count; i++)
        {
            ...
            var provider = _itemRepository.GetItemProvider(item.ID)!; // item is known to not be Empty
