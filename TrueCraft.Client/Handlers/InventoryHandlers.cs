...
internal static class InventoryHandlers
{
    ...
    public static void HandleOpenWindowPacket(IPacket _packet, MultiplayerClient client)
    {
        ...
        var windowID = packet.WindowID;
        var slotFactory = SlotFactory<ISlot>.Get();
        ...
        switch (packet.Type)
        {
            case WindowType.CraftingBench:
                window = new CraftingBenchWindow(
                    _itemRepository,
                    _craftingRepository,
                    slotFactory,
                    windowID,
            ...
            case WindowType.Chest:
                window = new ChestWindow(
                    _itemRepository,
                    slotFactory,
                    windowID,
                    client.Inventory,
                    client.Hotbar,
                    packet.TotalSlots == 2 * ChestWindow.ChestLength
                );

                break;

            case WindowType.Furnace:
                window = new FurnaceWindow(
                    _itemRepository,
                    slotFactory,
                    windowID,
                    client.Inventory,
    ...
    {
        var furnace = client.CurrentWindow as IFurnaceProgress;
        ...
        var progressPacket = (UpdateProgressPacket) packet;
    ...
    {
        var statusPacket = (TransactionStatusPacket) packet;
        var action = ActionList.Get(statusPacket.ActionNumber);
