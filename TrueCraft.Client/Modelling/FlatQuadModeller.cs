...
public abstract class FlatQuadModeller : BlockModeller
{
    ...
    public override VertexPositionNormalColorTexture[] Render(
        ...
        int indiciesOffset,
        out int[] indicies
    ) => RenderQuads(descriptor, offset, Texture, indiciesOffset, out indicies, Color.White);

    protected static VertexPositionNormalColorTexture[] RenderQuads(
        BlockDescriptor descriptor,
        Vector3 offset,
        Vector2[] textureMap,
        int indiciesOffset,
    ...
    )
    {
        ...
        {
            var quad = CreateAngledQuad(
                side,
                offset,
                textureMap,
                textureIndex % textureMap.Length,
                indiciesOffset,
                out _indicies,
                color
            );
    ...
    protected static VertexPositionNormalColorTexture[] CreateAngledQuad(
        int face,
        Vector3 offset,
        Vector2[] texture,
        int textureOffset,
