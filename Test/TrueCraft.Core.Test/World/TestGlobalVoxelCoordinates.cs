...
public class TestGlobalVoxelCoordinates
{
    ...
    {
        var actual = new GlobalVoxelCoordinates(x, y, z);
    ...
    {
        var expected = new GlobalVoxelCoordinates(x, y, z);

        var actual = new GlobalVoxelCoordinates(expected);
    ...
    [TestCase(
        (2 * WorldConstants.ChunkWidth) - 1,
        19,
        (2 * WorldConstants.ChunkDepth) - 1,
        1,
        1,
        WorldConstants.ChunkWidth - 1,
        19,
        WorldConstants.ChunkDepth - 1
    )]
    [TestCase(
        -1,
        21,
    ...
    public void From_Chunk_And_Local(
        int expectedX,
        int expectedY,
        int expectedZ,
        int chunkX,
    ...
    )
    {
        ...
        var local = new LocalVoxelCoordinates(localX, localY, localZ);

        var actual = GlobalVoxelCoordinates.GetGlobalVoxelCoordinates(chunk, local);
    ...
    {
        var a = new GlobalVoxelCoordinates(x1, y1, z1);
        var b = new GlobalVoxelCoordinates(x2, y2, z2);
    ...
    {
        var a = new GlobalVoxelCoordinates(2, 3, 5);
    ...
    {
        var a = new GlobalVoxelCoordinates(x, y, z);
    ...
    public void DistanceTo_GlobalVoxelCoordinates(int x1, int y1, int z1, int x2, int y2, int z2)
    {
        ...
        var dy = y2 - y1;
        var dz = z2 - z1;
        var expected = Math.Sqrt((dx * dx) + (dy * dy) + (dz * dz));
    ...
    public void Add(int x1, int y1, int z1, int x2, int y2, int z2)
    {
        ...
        var expected = new GlobalVoxelCoordinates(x1 + x2, y1 + y2, z1 + z2);
    ...
    public void Subtract(int x1, int y1, int z1, int x2, int y2, int z2)
    {
        ...
        var expected = new Vector3i(dx, dy, dz);
    ...
    public void Convert_GlobalColumnCoordinates(int x, int z)
    {
        ...
        var actual = (GlobalVoxelCoordinates) a;
    ...
    public void Convert_Vector3(double x, double y, double z)
    {
        ...
        var expected = new GlobalVoxelCoordinates((int) x, (int) y, (int) z);
    ...
    {
        var a = new GlobalChunkCoordinates(chunkX, chunkZ);
        var expected = new GlobalVoxelCoordinates(expectedX, 0, expectedZ);
