...
public sealed class FakeChunk : IChunk
{
    ...
    public FakeChunk(GlobalChunkCoordinates coordinates)
        : this(coordinates, DefaultSurfaceHeight) { }
    ...
    public FakeChunk(GlobalChunkCoordinates coordinates, byte surfaceHeight)
    {
        ...
        for (var z = 0; z < WorldConstants.ChunkDepth; z++)
        {
            ...
            for (var y = 1; y < _surfaceHeight; y++)
            {
    ...
    private static int CoordinatesToIndex(int x, int y, int z) => (((x * WorldConstants.ChunkWidth) + z) * WorldConstants.Height) + y;
    ...
    public byte GetBlockID(LocalVoxelCoordinates coordinates) => _blocks[CoordinatesToIndex(coordinates)];

    public byte GetBlockLight(LocalVoxelCoordinates coordinates) => _blockLight[CoordinatesToIndex(coordinates)];

    public int GetHeight(int x, int z) => _heightMap[x, z];

    public byte GetMetadata(LocalVoxelCoordinates coordinates) => _metadata[CoordinatesToIndex(coordinates)];

    public byte GetSkyLight(LocalVoxelCoordinates coordinates) => _skyLight[CoordinatesToIndex(coordinates)];

    public NbtCompound GetTileEntity(LocalVoxelCoordinates coordinates) => throw new NotImplementedException();
    ...
    public void SetMetadata(LocalVoxelCoordinates coordinates, byte value) => _metadata[CoordinatesToIndex(coordinates)] = value;
    ...
    public void SetTileEntity(LocalVoxelCoordinates coordinates, NbtCompound? value) => throw new NotImplementedException();

    public void UpdateHeightMap() => throw new NotImplementedException("Fix call to obsolete method.");
}