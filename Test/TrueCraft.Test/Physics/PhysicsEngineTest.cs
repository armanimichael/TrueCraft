...
public class PhysicsEngineTest
{
    ...
    private const byte _stoneBlockId = 1;

    private const int _surfaceHeight = 1;
    ...
    public PhysicsEngineTest()
    {
        ...
        mockStoneBlock.Setup(x => x.ID).Returns(_stoneBlockId);
        ...
        mockBlockRepository.Setup(x => x.GetBlockProvider(It.Is<byte>(b => b == _stoneBlockId)))
                           .Returns(mockStoneBlock.Object);
    ...
    private FakeDimension BuildDimension()
    {
        ...
        {
            chunk?.SetBlockID(new LocalVoxelCoordinates(x, 0, z), _stoneBlockId);
        }
    ...
    private sealed class TestEntity : IEntity
    {
        ...
        public BoundingBox BoundingBox
        {
            get
            {
                var hw = Size.Width * 0.5;
                var hd = Size.Depth * 0.5;
                var min = new Vector3(Position.X - hw, Position.Y, Position.Z - hd);
                var max = new Vector3(Position.X + hw, Position.Y + Size.Height, Position.Z + hd);
        ...
        public void Update(IEntityManager entityManager) => throw new NotImplementedException();
    ...
    public void TestGravity()
    {
        ...
        var physics = new PhysicsEngine(dimension);
    ...
    public void TestTerrainCollision()
    {
        ...
        Assert.True(Math.Abs(entity.Velocity.X) < _epsilon);
        Assert.True(Math.Abs(entity.Velocity.Y) < _epsilon);
        Assert.True(Math.Abs(entity.Velocity.Z) < _epsilon);
    ...
    public void TestInTerrainSupport()
    {
        ...
        for (var z = 0; z < WorldConstants.ChunkDepth; z++)
        {
            ...
            dimension.SetBlockID(coords, _stoneBlockId);
        }
    ...
    public void TestExtremeTerrainCollision()
    {
        ...
        entity.Position = new Vector3(0, _surfaceHeight + origHeightAboveSurface, 0);
        entity.Velocity = Vector3.Zero;
        ...
        Assert.AreEqual(0, entity.Position.X);
        Assert.AreEqual(_surfaceHeight, entity.Position.Y);
        Assert.AreEqual(0, entity.Position.Z);
    ...
    public void TestAdjacentFall()
    {
        ...
        for (var y = 0; y < 12; y++)
        {
            dimension.SetBlockID(new GlobalVoxelCoordinates(1, y, 0), _stoneBlockId);
        }
    ...
    public void TestCornerFall()
    {
        ...
        var bc1 = new GlobalVoxelCoordinates(5, _surfaceHeight, 5);
        var bc2 = new GlobalVoxelCoordinates(6, _surfaceHeight, 6);

        dimension.SetBlockID(bc1, _stoneBlockId);
        dimension.SetBlockID(bc2, _stoneBlockId);

        var positionBefore = new Vector3(
            bc1.X + 1 + (entity.Size.Width * 0.5),
            _surfaceHeight + 0.9,
            bc2.Z - (entity.Size.Depth * 0.5)
        );
    ...
    public void CrazyJump()
    {
        ...
        var chunk12 = dimension.GetChunk(new GlobalChunkCoordinates(-1, 12), LoadEffort.Generate)!;
        var chunk13 = dimension.GetChunk(new GlobalChunkCoordinates(-1, 13), LoadEffort.Generate)!;
        ...
        for (var z = 0; z < 16; z++)
        {
            ...
            chunk12.SetBlockID(localCoords, _stoneBlockId);
            chunk13.SetBlockID(localCoords, _stoneBlockId);
        }

        // Place blocks at z == 192, x == -5 && x == -3
        dimension.SetBlockID(new GlobalVoxelCoordinates(-4, 63, 192), _stoneBlockId);
        dimension.SetBlockID(new GlobalVoxelCoordinates(-6, 63, 192), _stoneBlockId);

        var entitySize = new Size(0.6, 1.8, 0.6); // same size as Player Entity
        var xPos = -4.34298322997483;
        double yPos = 63;
        var zPos = 192.92777590726;
        var entityStartPos = new Vector3(xPos, yPos, zPos);
        var xVel = 0.0502763610985;
        double yVel = 0;
        var zVel = -0.0520632516009488;
        var entityStartVel = new Vector3(xVel, yVel, zVel);
        ...
        Assert.True(
            Math.Abs(zPos + (2 * zVel) - entity.Position.Z) < GameConstants.Epsilon,
            "Expected: {0};  Actual: {0}",
            zPos + (2 * zVel),
            entity.Position.Z
        );
    ...
    )
    {
        ...
        //
        Assert.True(
            Math.Abs(expectedPosition.X - entity.Position.X) < GameConstants.Epsilon,
            $"X Position: Expected: {expectedPosition.X}; Actual: {entity.Position.X}"
        );

        Assert.True(
            Math.Abs(expectedPosition.Y - entity.Position.Y) < GameConstants.Epsilon,
            $"Y Position: Expected: {expectedPosition.Y}; Actual: {entity.Position.Y}"
        );

        Assert.True(
            Math.Abs(expectedPosition.Z - entity.Position.Z) < GameConstants.Epsilon,
            $"Z Position: Expected: {expectedPosition.Z}; Actual: {entity.Position.Z}"
        );

        Assert.True(
            Math.Abs(expectedDirection.X - entity.Velocity.X) < GameConstants.Epsilon,
            $"X Direction: Expected: {expectedDirection.X}; Actual: {entity.Velocity.X}"
        );

        Assert.True(
            Math.Abs(expectedDirection.Y - entity.Velocity.Y) < GameConstants.Epsilon,
            $"Y Direction: Expected: {expectedDirection.Y}; Actual: {entity.Velocity.Y}"
        );

        Assert.True(
            Math.Abs(expectedDirection.Z - entity.Velocity.Z) < GameConstants.Epsilon,
    ...
    public void WalkIntoCorner2()
    {
        ...
        // Build cliff
        var chunk = dimension.GetChunk(new GlobalChunkCoordinates(0, 0), LoadEffort.Generate)!;

        for (var x = 7; x < WorldConstants.ChunkWidth; x++)
        for (var z = 0; z < WorldConstants.ChunkDepth; z++)
        for (var y = 1; y < 7; y++)
        {
            chunk.SetBlockID(new LocalVoxelCoordinates(x, y, z), _stoneBlockId);
        }
        ...
        var entitySize = new Size(0.6, 1.8, 0.6); // same size as Player Entity
        var entity = new TestEntity();
        var startPosition = new Vector3(9.658, 3, 7.3);
        var startDirection = new Vector3(3.064, 0, -3.118);
        entity.Position = startPosition;
        entity.Velocity = startDirection;
        entity.Size = entitySize;
        entity.AccelerationDueToGravity = (float) GameConstants.AccelerationDueToGravity;
    ...
    public void IsGrounded(int serial, bool expected, double x, double y, double z)
    {
        ...
        var actual = physics.IsGrounded(entity);
    ...
    public void IsGrounded_Edge()
    {
        var dimension = BuildDimension();
        var physics = new PhysicsEngine(dimension);
        var bc = new GlobalVoxelCoordinates(5, 64, 7);

        dimension.SetBlockID(bc, _stoneBlockId);
    ...
    public void IsGrounded_Corner()
    {
        var dimension = BuildDimension();
        var physics = new PhysicsEngine(dimension);
        GlobalVoxelCoordinates bc = new(5, _surfaceHeight + 1, 5);

        for (var x = bc.X - 1; x <= bc.X + 1; x++)
        {
            dimension.SetBlockID(new GlobalVoxelCoordinates(x, bc.Y, bc.Z), _stoneBlockId);
        }

        for (var z = bc.Z - 1; z <= bc.Z + 1; z++)
        {
            dimension.SetBlockID(new GlobalVoxelCoordinates(bc.X, bc.Y, z), _stoneBlockId);
        }

        TestEntity entity = new();
        entity.Size = new Size(0.6, 1.6, 0.6);
        var hw = entity.Size.Width * 0.5;
        var hd = entity.Size.Depth * 0.5;
        physics.AddEntity(entity);
    ...
    public void JumpUpOneBlock()
    {
        ...
        var chunk = dimension.GetChunk(chunkCoordinates, LoadEffort.Generate)!;
        ...
        for (var z = 0; z < WorldConstants.ChunkDepth; z++)
        {
            chunk.SetBlockID(new LocalVoxelCoordinates(x, yLower, z), _stoneBlockId);
        }

        for (var x = 0; x < WorldConstants.ChunkWidth; x++)
        for (var z = 9; z < WorldConstants.ChunkDepth; z++) // 9 == 201 mod 16
        {
            chunk.SetBlockID(new LocalVoxelCoordinates(x, yUpper, z), _stoneBlockId);
        }
        ...
        var initialPosition = new Vector3(-8.401511844263727, 64.05635210306419, 201.06527613705063);
    ...
    public void EmbeddedJump()
    {
        ...
        for (var z = 0; z < WorldConstants.ChunkDepth; z++)
        {
            chunk.SetBlockID(new LocalVoxelCoordinates(x, 1, z), _stoneBlockId);
        }

        var bc = new LocalVoxelCoordinates(5, 1, 5);
