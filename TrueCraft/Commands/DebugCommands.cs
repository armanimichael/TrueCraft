...
public class SaveCommand : Command
{
    ...
    public override string Description => "Saves the dimension!";
    ...
    public override void Help(IRemoteClient client, string alias, string[] arguments) => client.SendMessage("/save: Saves the dimension!");
}
...
public class SkyLightCommand : Command
{
    ...
    public override void Handle(IRemoteClient client, string alias, string[] arguments)
    {
        ...
        client.SendMessage(
            client.Dimension!.GetSkyLight(
                      (GlobalVoxelCoordinates) (client.Entity!.Position + new Vector3(0, -mod, 0))
                  )
                  .ToString()
        );
    }
...
public class SpawnCommand : Command
{
    ...
    public override void Handle(IRemoteClient client, string alias, string[] arguments)
    {
        ...
        var em = ((IDimensionServer) client.Dimension!).EntityManager;
...
public class ToMeCommand : Command
{
    ...
    public override void Handle(IRemoteClient client, string alias, string[] arguments)
    {
        ...
        var manager = ((IDimensionServer) client.Dimension!).EntityManager;
        ...
        Task.Factory.StartNew(
            () =>
            {
                ...
                var path = astar.FindPath(
                    ...
                    (GlobalVoxelCoordinates) client.Entity!.Position
                );
                ...
                else
                {
                    client.SendMessage(
                        string.Format(
                            ChatColor.Blue
                            + "Executing path with {0} waypoints",
                            path.Count
                        )
                    );
...
public class EntityInfoCommand : Command
{
    ...
    public override void Help(IRemoteClient client, string alias, string[] arguments) => client.SendMessage("/entity [id]: Shows information about this entity.");
}
    ...
    public override void Handle(IRemoteClient client, string alias, string[] arguments)
    {
        if (arguments.Length != 0)
        {
            ...
            else if (arguments[0] == "all")
            {
                ...
                for (var i = 0; i < client.Hotbar.Count; i++)
                {
                    client.Hotbar[i].Item = ItemStack.EmptyStack;
                }

                for (var i = 0; i < client.Inventory.Count; i++)
                {
                    client.Inventory[i].Item = ItemStack.EmptyStack;
                }

                for (var i = 0; i < client.Armor.Count; i++)
                {
                    client.Armor[i].Item = ItemStack.EmptyStack;
                }

                for (var i = 0; i < client.CraftingGrid.Count; i++)
                {
...
public class LogCommand : Command
{
    ...
    public override void Help(IRemoteClient client, string alias, string[] arguments) => client.SendMessage("/log: Toggles client logging.");
}
...
public class ResendInvCommand : Command
{
    ...
    public override void Handle(IRemoteClient client, string alias, string[] arguments)
    {
        ...
        var items = new ItemStack[client.Inventory.Count + client.Hotbar.Count];
        var idx = 0;
        int j;
...
public class RelightCommand : Command
{
    ...
    public override void Handle(IRemoteClient client, string alias, string[] arguments)
    {
        ...
        var server = (MultiplayerServer) client.Server;
        var chunk = client.Dimension!.GetChunk((GlobalVoxelCoordinates) client.Entity!.Position)!;
        var lighter = server.WorldLighters.SingleOrDefault(l => l.Dimension == client.Dimension);
    ...
    public override void Help(IRemoteClient client, string alias, string[] arguments) => client.SendMessage("/relight: Relights the chunk you're standing in.");
}