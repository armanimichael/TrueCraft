...
public class FurnaceWindow : Window<IServerSlot>,
    IFurnaceWindow<IServerSlot>,
    IServerWindow
{
    ...
    public FurnaceWindow(
        ...
        base(
            ...
            windowID,
            Core.Windows.WindowType.Furnace,
            "Furnace",
            new ISlots<IServerSlot>[]
            {
                new ServerSlots(serviceLocator.ItemRepository, new List<IServerSlot> { furnaceSlots.IngredientSlot }),
    ...
    public void HandleClick(IRemoteClient client, ClickWindowPacket packet)
    {
        ...
        if (handled)
        {
            ...
            var furnace = (FurnaceBlock) _serviceLocator.BlockRepository.GetBlockProvider(FurnaceBlock.BlockID);
    ...
    protected bool HandleLeftClick(int slotIndex, ref ItemStack itemStaging)
    {
        if (IsOutputSlot(slotIndex))
        {
            ...
            var numToPickUp = Math.Min(maxStack - itemStaging.Count, output.Count);
        ...
        {
            int maxStack =
                ItemRepository.GetItemProvider(itemStaging.ID)!.MaximumStack; // itemStaging is known to not be Empty

            var numToPlace = Math.Min(maxStack - slotContent.Count, itemStaging.Count);
    ...
    protected bool HandleRightClick(int slotIndex, ref ItemStack itemStaging)
    {
        ...
        if (IsOutputSlot(slotIndex))
        {
            ...
            if (itemStaging.Count < maxStack)
            {
                ...
                var amt = (sbyte) (output.Count + itemStaging.Count > maxStack
                    ? maxStack - itemStaging.Count
                    : output.Count);
