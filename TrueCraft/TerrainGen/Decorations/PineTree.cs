...
public class PineTree : Decoration
{
        ...
        for (var y = 1; y < height; y++)
        {
            if (y % 2 == 0)
            {
                GenerateVanillaCircle(
                    chunk,
                    new LocalVoxelCoordinates(location.X, location.Y + y + 1, location.Z),
                    LeafRadius - 1,
                    LeavesBlock.BlockID,
                    0x1
                );
     ...
     */
    protected static void GenerateTopper(IChunk chunk, LocalVoxelCoordinates location, byte type = 0x0)
    {
        const int sectionRadius = 1;
        GenerateCircle(chunk, location, sectionRadius, LeavesBlock.BlockID, 0x1);
        var top = new LocalVoxelCoordinates(location.X, location.Y + 1, location.Z);
