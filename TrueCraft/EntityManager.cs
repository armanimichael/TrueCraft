...
public class EntityManager : IEntityManager
{
    ...
    private object _entityLock = new();
    ...
    private void HandlePlayerPropertyChanged(string property, PlayerEntity entity)
    {
        ...
        switch (property)
        {
            case "Position":
                ...
                {
                    client.Log(
                        "Passed chunk boundary at {0}, {1}",
                        (int) entity.Position.X >> 4,
                        (int) entity.Position.Z >> 4
                    );

                    _server.Scheduler.ScheduleEvent(
                        "client.update-chunks",
                        client,
                        TimeSpan.Zero,
                        s => client.UpdateChunks()
                    );
    ...
    internal void UpdateClientEntities(RemoteClient client)
    {
        ...
        for (var i = 0; i < client.KnownEntities.Count; i++)
        {
            ...
            if (knownEntity.Position.DistanceTo(entity.Position) > client.ChunkRadius * WorldConstants.ChunkDepth)
            {
                ...
                {
                    var c = (RemoteClient?) GetClientForEntity((PlayerEntity) knownEntity);
        ...
        foreach (var e in toSpawn)
        {
            if (e != entity && !client.KnownEntities.Contains(e))
            {
                ...
                {
                    var c = (RemoteClient?) GetClientForEntity((PlayerEntity) e);
    ...
    private static bool IsInRange(Vector3 a, Vector3 b, int range) => Math.Abs(a.X - b.X) < range * WorldConstants.ChunkWidth &&
                                                               Math.Abs(a.Z - b.Z) < range * WorldConstants.ChunkDepth;

    private IEntity[] GetEntitiesInRange(IEntity entity, int maxChunks) => _entities.Where(
                                                                                        e =>
                                                                                            e.EntityID != entity.EntityID && !e.Despawned && IsInRange(
                                                                                                e.Position,
                                                                                                entity.Position,
                                                                                                maxChunks
                                                                                            )
    ...
    private void SendEntityToClient(RemoteClient client, IEntity entity)
    {
        ...
        client.Log(
            "Spawning entity {0} ({1}) at {2}",
            entity.EntityID,
            entity.GetType().Name,
            (GlobalVoxelCoordinates) entity.Position
        );
        ...
        if (spawnedClient is not null)
        {
            ...
            client.QueuePacket(
                new EntityEquipmentPacket(
                    entity.EntityID,
                    0,
                    spawnedClient.SelectedItem.ID,
                    spawnedClient.SelectedItem.Metadata
                )
            );
            ...
            client.QueuePacket(
                new EntityEquipmentPacket(
                    entity.EntityID,
                    4,
                    spawnedClient.Armor[0].Item.ID,
                    spawnedClient.Armor[0].Item.Metadata
                )
            );
            ...
            client.QueuePacket(
                new EntityEquipmentPacket(
                    entity.EntityID,
                    3,
                    spawnedClient.Armor[1].Item.ID,
                    spawnedClient.Armor[1].Item.Metadata
                )
            );
            ...
            client.QueuePacket(
                new EntityEquipmentPacket(
                    entity.EntityID,
                    2,
                    spawnedClient.Armor[2].Item.ID,
                    spawnedClient.Armor[2].Item.Metadata
                )
            );
            ...
            client.QueuePacket(
                new EntityEquipmentPacket(
                    entity.EntityID,
                    1,
                    spawnedClient.Armor[3].Item.ID,
                    spawnedClient.Armor[3].Item.Metadata
                )
            );
        }
    }

    private IRemoteClient? GetClientForEntity(PlayerEntity entity) => _server.Clients.SingleOrDefault(c => c.Entity is not null && c.Entity.EntityID == entity.EntityID);

    public IList<IEntity> EntitiesInRange(Vector3 center, float radius) => _entities.Where(e => !e.Despawned && e.Position.DistanceTo(center) < radius).ToList();

    public IList<IRemoteClient> ClientsForEntity(IEntity entity) => _server.Clients.Where(c => ((RemoteClient) c).KnownEntities.Contains(entity)).ToList();
    ...
    public void FlushDespawns()
    {
        ...
        while (!_pendingDespawns.IsEmpty)
        {
            ...
            if (entity is not null)
            {
                ...
                lock (((MultiplayerServer) _server)
                      .ClientLock) // TODO: Thread safe way to iterate over client collection
                {
                    for (int i = 0, ServerClientsCount = _server.Clients.Count; i < ServerClientsCount; i++)
                    {
                        var client = (RemoteClient) _server.Clients[i];
    ...
    public IEntity? GetEntityByID(int id) => _entities.SingleOrDefault(e => e.EntityID == id);
