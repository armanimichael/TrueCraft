...
internal static class LoginHandlers
{
    ...
    public static void HandleLoginRequestPacket(IPacket packet, IRemoteClient client, IMultiplayerServer server)
    {
        ...
        else
        {
            ...
            var dimension =
                (IDimensionServer) ((IWorld) server.World)[
                    DimensionID.Overworld]; // TODO read dimension from saved player data.
            ...
            var collision = new Func<bool>(
                () =>
                {
                    var feet = client.Dimension!.GetBlockID((GlobalVoxelCoordinates) client.Entity!.Position);
                    var head = client.Dimension.GetBlockID((GlobalVoxelCoordinates) (client.Entity.Position + Vector3.Up));
            ...
            var entityManager = ((IDimensionServer) remoteClient.Dimension).EntityManager;
            ...
            remoteClient.QueuePacket(
                new SpawnPositionPacket(
                    (int) remoteClient.Entity.Position.X,
                    (int) remoteClient.Entity.Position.Y,
                    (int) remoteClient.Entity.Position.Z
                )
            );

            remoteClient.QueuePacket(
                new SetPlayerPositionPacket(
                    remoteClient.Entity.Position.X,
                    remoteClient.Entity.Position.Y + 1,
                    remoteClient.Entity.Position.Y + remoteClient.Entity.Size.Height + 1,
                    remoteClient.Entity.Position.Z,
                    remoteClient.Entity.Yaw,
                    remoteClient.Entity.Pitch,
                    true
                )
            );
