...
public static class InteractionHandlers
{
    ...
    public static void HandlePlayerDiggingPacket(IPacket _packet, IRemoteClient _client, IMultiplayerServer server)
    {
        ...
        var dimension = _client.Dimension!;
        ...
        switch (packet.PlayerAction)
        {
            case PlayerDiggingPacket.Action.DropItem:
                ...
                item.Velocity = MathHelper.RotateY(Vector3.Forwards, MathHelper.DegreesToRadians(client.Entity.Yaw)) *
                                0.5;
            ...
            case PlayerDiggingPacket.Action.StartDigging:
                foreach (var nearbyClient in
                         server.Clients) // TODO: Send this repeatedly during the course of the digging
                {
            ...
            case PlayerDiggingPacket.Action.StopDigging:
                foreach (var nearbyClient in server.Clients)
                {
                    ...
                    {
                        c.QueuePacket(
                            new AnimationPacket(client.Entity!.EntityID, AnimationPacket.PlayerAnimation.None)
                        );
                    }
                }
                ...
                {
                    time = BlockProvider.GetHarvestTime(
                        _serviceLocator,
                        descriptor.ID,
                    ...
                    if (diff > -100) // Allow a small tolerance
                    {
                        ...
                        {
                            var tool =
                                _serviceLocator.ItemRepository.GetItemProvider(client.SelectedItem.ID) as IDurableItem;
    ...
    public static void HandlePlayerBlockPlacementPacket(
        IPacket _packet,
    ...
    )
    {
        ...
        if (block != null)
        {
            ...
            {
                server.SendMessage(
                    ChatColor.Red + "WARNING: block provider for ID {0} is null (player placing)",
                    block.Value.ID
                );

                server.SendMessage(
                    ChatColor.Red + "Error occured from client {0} at coordinates {1}",
                    client.Username ?? "?",
                    block.Value.Coordinates
                );
        ...
        if (!slot.Empty)
        {
            if (use)
            {
                ...
                {
                    server.SendMessage(
                        ChatColor.Red + "WARNING: item provider for ID {0} is null (player placing)",
                        block?.ID.ToString() ?? "(null)"
                    );

                    server.SendMessage(
                        ChatColor.Red + "Error occured from client {0} at coordinates {1}",
                        client.Username ?? "?",
                        block?.Coordinates.ToString() ?? "(null)"
                    );
    ...
    public static void HandleClickWindowPacket(IPacket _packet, IRemoteClient _client, IMultiplayerServer server)
    {
        ...
        {
            server.Log(
                Core.Logging.LogCategory.Notice,
                "Invalid window number received {0}; expected {1}",
                packet.WindowID,
                window.WindowID
            );
        ...
        {
            var dimension = (IDimensionServer) client.Dimension!;
            var entityManager = dimension.EntityManager;
            // Throwing item
            ItemEntity item;
        ...
        if (packet.SlotIndex >= window.Count || packet.SlotIndex < 0)
        {
            server.Log(
                Core.Logging.LogCategory.Notice,
                "Illegal slot number received {0} in not in the set -999, [0, {1})",
                packet.SlotIndex,
                window.Count
            );
    ...
    public static void HandleChangeHeldItem(IPacket _packet, IRemoteClient _client, IMultiplayerServer server)
    {
        ...
        var notified = ((IDimensionServer) client.Dimension!).EntityManager.ClientsForEntity(client.Entity!);
    ...
    public static void HandleAnimation(IPacket _packet, IRemoteClient _client, IMultiplayerServer server)
    {
        ...
        {
            var nearby = ((IDimensionServer) client.Dimension!).EntityManager
    ...
    public static void HandleUpdateSignPacket(IPacket _packet, IRemoteClient _client, IMultiplayerServer server)
    {
        ...
        if (client.Entity!.Position.DistanceTo((Vector3) coords) < 10) // TODO: Reach
        {
            ...
            {
                ((IDimensionServer) client.Dimension).SetTileEntity(
                    coords,
                    new NbtCompound(
                        new[]
                        {
                ...
                server.Clients.Where(c => ((RemoteClient) c).LoggedIn && c.Dimension == _client.Dimension)
                      .ToList()
