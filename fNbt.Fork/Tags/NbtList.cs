...
public sealed class NbtList : NbtTag, IList<NbtTag>, IList
{
    ...
    {
        Name = tagName;
        this._tags = new List<NbtTag>();
        _listType = givenListType;

        if (!Enum.IsDefined(givenListType))
        {
    ...
    public override NbtTag this[int tagIndex]
    {
        ...
        set
        {
            ...
            {
                _listType = value.TagType;
            }
            else if (value.TagType != _listType)
            {
    ...
    [Pure]
    public T Get<T>(int tagIndex)
        where T : NbtTag => (T) _tags[tagIndex];
    ...
    public void Insert(int tagIndex, NbtTag newTag)
    {
        ...
        else if (newTag.TagType != _listType)
        {
            throw new ArgumentException("Items must be of type " + _listType);
        }
    ...
    public void Add(NbtTag newTag)
    {
        ...
        else if (newTag.TagType != _listType)
        {
            throw new ArgumentException(
                "Items in this list must be of type " + _listType + ". Given type: " +
                newTag.TagType
            );
        }
        else if (newTag.Parent != null)
    ...
    bool IList.Contains(object value) => _tags.Contains((NbtTag) value);

    int IList.IndexOf(object value) => _tags.IndexOf((NbtTag) value);

    void IList.Insert(int index, object value) => Insert(index, (NbtTag) value);
    ...
    private new NbtTag this[string tagName]
    {
